...
function admin_addDel() {
	...
	ES_URL = "/hkmc/ajax/ES_admin.jsp?type=admin_addDel";
...
//	var language = $("#updateAdmin_language").val();
	...
	if(!adminDomainUpdateFlag){
		...
		$.ajax({
			type:"POST",
			url: "http://10.10.10.229:9200/hkmc_usergroup_info/hkmc_usergroup/"+adminid+"/_update",
			contentType: "application/json; charset=UTF-8",
			data: jsonData,
			dataType:"JSON",
			timeout: 30000,
...
function addDomain() {
	...
	$.ajax({
		type: "POST",
		url: "http://10.10.10.229:9200/hkmc_usergroup_info/hkmc_usergroup",
		contentType: "application/json; charset=UTF-8",
		data: jsonData,
		dataType: "JSON",
		timeout: 30000,
...
function delAdmin(idx,searchid) {
	...
	$.ajax({
		type: "DELETE",
		url: "http://10.10.10.229:9200/hkmc_usergroup_info/hkmc_usergroup/"+searchid,
		contentType: "application/json; charset=UTF-8",
		dataType: "JSON",
		timeout: 30000,
		success: function(data){
...
function getDomainAdminList(item) {
	...
	if(searchKeyword == '')
	{
		ES_URL = "/hkmc/ajax/ES_admin.jsp?type=getDomainAdminList&usertype="+usertype+"&offset="+offset+"&size="+size+"&searchKeyword=none";
		
		
	}else{
		ES_URL = "/hkmc/ajax/ES_admin.jsp?type=getDomainAdminList&usertype="+usertype+"&offset="+offset+"&size="+size+"&searchKeyword="+searchKeyword+"";
