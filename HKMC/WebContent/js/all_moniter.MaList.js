...
function all_MaList(){
	$(".content").empty();
	$(".content").load("all_MaList.jsp");
}

function all_GetInit(){
	//getMainMaStatTotal();
	//test();
}
function all_getMaList(item,title)
{
var crmdomainName = "";
...
//	console.log(crmdomain)
	...
	if (statFilter == "null" && categoryFilter == "null" )
	{
		...
		ES_URL = "/hkmc/ajax/ES_MaList.jsp?type=MaList&title="+title+"&status=none&t="+encodeURI(A)+"&offset="+offset+"&size="+size+"&array="+encodeURI(jsonString)+"&timeFrom=\""+timeFrom+"\"&timeTo=\""+timeTo+"\"";
	}
	else if ( statFilter == "null" )
	{
		var jsonString = "{\"type\":"+JSON.stringify(array)+"}";
		ES_URL = "/hkmc/ajax/ES_MaList.jsp?type=MaList&status=category&t="+encodeURI(A)+"&offset="+offset+"&size="+size+"&array="+encodeURI(jsonString)+"&categoryFilter="+categoryFilter+"&timeFrom=\""+timeFrom+"\"&timeTo=\""+timeTo+"\"";
	}
	else if ( categoryFilter == "null" )
	{
		var jsonString = "{\"type\":"+JSON.stringify(array)+"}";
		ES_URL = "/hkmc/ajax/ES_MaList.jsp?type=MaList&status=stat&t="+encodeURI(A)+"&offset="+offset+"&size="+size+"&array="+encodeURI(jsonString)+"&statFilter="+statFilter+"&timeFrom=\""+timeFrom+"\"&timeTo=\""+timeTo+"\"";
	}
	
	else
	{
		var jsonString = "{\"type\":"+JSON.stringify(array)+"}";
		ES_URL = "/hkmc/ajax/ES_MaList.jsp?type=MaList&status=all&t="+encodeURI(A)+"&offset="+offset+"&size="+size+"&array="+encodeURI(jsonString)+"&categoryFilter="+categoryFilter+"&statFilter="+statFilter+"&timeFrom=\""+timeFrom+"\"&timeTo=\""+timeTo+"\"";
	}
...
function all_getMaListSearch(item) {
...
//					console.log(test[i])
	...
	$.ajax({
		type:"POST",
		url: "/hkmc/ajax/ES_MaList.jsp?type=MaListSearch&t="+encodeURI(A)+"&offset="+offset+"&size="+size+"&searchKeyword=\"*"+searchKeyword+"*\"",
		contentType: "application/json; charset=UTF-8",
		dataType:"JSON",
		timeout: 30000,
		success : function(data) {
...
function all_getMaListDetailSearch(item) {
...
//					console.log(test[i])
	...
	$.ajax({
		type:"POST",
		url: "/hkmc/ajax/ES_MaList.jsp?type=MaListDetailSearch&t="+encodeURI(A)+"&offset="+offset+"&size="+size+"&filters=\""+filters+"\"&searchKeyword=\"*"+searchKeyword+"*\"&timeFrom=\""+timeFrom+"\"&timeTo=\""+timeTo+"\"",
		contentType: "application/json; charset=UTF-8",
		dataType:"JSON",
		timeout: 30000,
		success : function(data) {
...
function all_ajaxProcess(item,title){
	...
	if(maFlags == "getMaList"){
		$("#pageIdx").val(0);
		all_getMaList(item,title);
	}
	
	if(maFlags == "getMaListSearch")
	{
		$("#pageIdx").val(0);
		all_getMaListSearch(item)		
	}
	
	if(maFlags == "getMaListDetailSearch")
	{
		$("#pageIdx").val(0);
		all_getMaListDetailSearch(item)		
	}
}
...
function getMainMaStatTotal()
{
	...
	$.ajax({
		type:"POST",
		url: "/hkmc/ajax/ES_MaList.jsp?type=MainMaStatTotal",
		contentType: "application/json; charset=UTF-8",
		dataType:"text",
		timeout: 30000,
		success : function(data) {
...
function test()
{
	...
	$.ajax({
		type:"POST",
		url: "/hkmc/ajax/ES_MaList.jsp?type=MaListDetailSearch&offset=1&size=10&filters=\"장애\"&searchKeyword=\"**\"&timeFrom=\""+timeFrom+"\"&timeTo=\""+timeTo+"\"",
		contentType: "application/json; charset=UTF-8",
		dataType:"text",
		timeout: 30000,
		success : function(data) {
