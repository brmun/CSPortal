...
function getProjectList(item,title)
{
	...
	if (statFilter == "null" && categoryFilter == "null" )
	{
		...
		ES_URL = "/hkmc/ajax/ES_ProjectList.jsp?type=getProjectList&title="+title+"&status=none&t="+encodeURI(A)+"&array="+encodeURI(jsonString)+"&offset="+offset+"&size="+size+"";
	}
	else if ( statFilter == "null" )
	{
		var jsonString = "{\"type\":"+JSON.stringify(array)+"}";
		ES_URL = "/hkmc/ajax/ES_ProjectList.jsp?type=getProjectList&status=category&t="+encodeURI(A)+"&array="+encodeURI(jsonString)+"&offset="+offset+"&size="+size+"&categoryFilter="+categoryFilter+"";
	}
	else if ( categoryFilter == "null" )
	{
		var jsonString = "{\"type\":"+JSON.stringify(array)+"}";
		ES_URL = "/hkmc/ajax/ES_ProjectList.jsp?type=getProjectList&status=stat&t="+encodeURI(A)+"&array="+encodeURI(jsonString)+"&offset="+offset+"&size="+size+"&statFilter="+statFilter+"";
	}
	
	else
	{
		var jsonString = "{\"type\":"+JSON.stringify(array)+"}";
		ES_URL = "/hkmc/ajax/ES_ProjectList.jsp?type=getProjectList&status=all&t="+encodeURI(A)+"&array="+encodeURI(jsonString)+"&offset="+offset+"&size="+size+"&categoryFilter="+categoryFilter+"&statFilter="+statFilter+"";
	}
	$.ajax({
		type:"POST",
		url:  ES_URL,
...
function getProjectContent(PROJECT_ID,PROJECT_NAME,POTENTIAL_ID)
{
	...
	$.ajax({
		type:"POST",
		url:  "/hkmc/ajax/ES_ProjectList.jsp?type=getProjectContent&t=null&PROJECT_ID="+PROJECT_ID+"",
		contentType: "application/json; charset=UTF-8",
		dataType:"text",
		timeout: 30000,
		success : function(data) {
...
function tickets(POTENTIAL_ID)
{
	...
	$.ajax({
		type:"POST",
		url:  "/hkmc/ajax/ES_ProjectList.jsp?type=tickets&t=null&PROJECT_ID="+POTENTIAL_ID+"&array="+encodeURI(jsonString),
		contentType: "application/json; charset=UTF-8",
		dataType:"text",
		timeout: 30000,
		success : function(data) {
