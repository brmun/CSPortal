...
<%-- 		console.log("<%=URLDecoder.decode(domain,"UTF-8")%>");	 --%>
		ES_URL = "/admin/ajax/ES_login.jsp?type=PWCH&email="+email+"";
	...
	function domain_list(email,right) {
		...
		ES_URL = "/admin/ajax/ES_admin.jsp?type=header";
...
//		html += "        </ul>";
	...
	function findDomain()
	{
		...
		$.ajax({
			type:"POST",
			url: "/admin/ajax/ES_login.jsp?type=domain_search&searchKeyword=\"*"+searchKeyword+"*\"",
			contentType: "application/json; charset=UTF-8",
			dataType:"JSON",
			timeout: 30000,
			success : function(data) {
