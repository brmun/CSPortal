...
<%-- //  		jsonData = JSON.stringify({"id":email, "password":pwd, "userIp":"<%=request.getRemoteAddr()%>"}); --%>
  		...
  		ES_URL = "/admin/ajax/ES_login.jsp?type=checkIDPW&email="+email+"";
						...
						if ( key == "hits" )
						{
							...
							else
							{
								if (hits[0]._source.userpw == pwd)
								{
										...
										ES_URL = "/admin/ajax/ES_login.jsp?type=checkIDPW2&email="+email+"&domain="+encodeURI(encodeURIComponent(domain))+"";
								  			...
								  			success : function(data) {
												...
												$.each(data, function(key, value) {
													if ( key == "hits" )
													{
														...
														ES_URL = "/admin/ajax/ES_login.jsp?type=login_log&logtype=login&userid="+email+"&ComPany_name="+domain+"&log_time="+today+"";
														$.ajax({
												  			type:"POST",
												  			url: ES_URL,
												  			contentType: "application/json; charset=UTF-8",
