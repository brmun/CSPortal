...
function admin_addDel() {
	...
	ES_URL = "/admin/ajax/ES_admin.jsp?type=admin_addDel";
...
//	var language = $("#updateAdmin_language").val();
	...
	if(!adminDomainUpdateFlag){
		...
		$.ajax({
			type:"POST",
			url: "http://10.10.10.229:9200/sc_usergroup_info/sc_usergroup/"+adminid+"/_update",
			contentType: "application/json; charset=UTF-8",
			data: jsonData,
			dataType:"JSON",
			timeout: 30000,
...
//			"timeCreated" : ""+today+""
//	});
		confirmPassword = encodeURIComponent(confirmPassword).replace(/'/g,"%27").replace(/"/g,"%22");
		domain = encodeURIComponent(domain).replace(/'/g,"%27").replace(/"/g,"%22");
		
		ES_URL = "/admin/ajax/ES_admin.jsp?type=addDomain&division=2&userid="+newId+"&username="+newname+"&userpw="+confirmPassword+"&domain="+domain+"&right="+right+"&timeCreated="+today+"\"";
		console.log("domain not null : "+ES_URL)
	$.ajax({
		type: "POST",
		url: ES_URL,
...
//	});
		...
		domain = encodeURIComponent(domain).replace(/'/g,"%27").replace(/"/g,"%22");
		ES_URL = "/admin/ajax/ES_admin.jsp?type=addDomain&division=2&userid="+newId+"&username="+newname+"&userpw="+confirmPassword+"&domain="+domain+"&right="+right+"&timeCreated="+today+"\"";
		console.log("domain null : "+ES_URL)
		$.ajax({
			type: "POST",
			url: ES_URL,
	//		url: "http://10.10.10.229:9200/sc_usergroup_info/sc_usergroup",
...
function delAdmin(idx,searchid) {
	...
	$.ajax({
		type: "DELETE",
		url: "http://10.10.10.229:9200/sc_usergroup_info/sc_usergroup/"+searchid,
		contentType: "application/json; charset=UTF-8",
		dataType: "JSON",
		timeout: 30000,
		success: function(data){
...
function getDomainAdminList(item) {
	...
	if(searchKeyword == '')
	{
		ES_URL = "/admin/ajax/ES_admin.jsp?type=getDomainAdminList&usertype="+usertype+"&offset="+offset+"&size="+size+"&searchKeyword=none";
		
		
	}else{
		ES_URL = "/admin/ajax/ES_admin.jsp?type=getDomainAdminList&usertype="+usertype+"&offset="+offset+"&size="+size+"&searchKeyword="+searchKeyword+"";
